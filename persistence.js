...
async function hydrate(localData){
 ...
 (attempts.data||[]).forEach(a=>{const legacyName=Object.entries(window.RainbowCurriculum?.aliases||{}).find(([,id])=>id===a.skill_id)?.[0]||a.skill_id;const key=`${a.domain}::${legacyName}`;(evidence[key]||(evidence[key]=[])).push({date:a.timestamp?.slice(0,10),score:a.result==='CORRECT'?1:a.result==='PARTIAL'?.65:.3,activityId:a.activity_id});questHistory.push({date:a.timestamp?.slice(0,10),activityId:a.activity_id,score:a.result==='CORRECT'?1:.4,interaction_type:a.interaction_type,hints:a.hint_level_used,attempts:a.number_of_attempts});});
