...
const PENDING_KEY='sakhiPendingProgressV1';
...
async function init(){
 ...
 if(remote){await ensureProfile();const pending=JSON.parse(localStorage.getItem(PENDING_KEY)||'null');if(pending)await syncSnapshot(pending);}
 return {remote};
}
...
async function signOut(){if(client){const result=await client.auth.signOut();if(result?.error)throw result.error;}remote=false;learnerId=null;sessionId=null;}
...
async function syncSkills(data){if(!remote||!learnerId)return;const rows=[];(window.RainbowCurriculum?.skills||[]).forEach(s=>{const state=stateFor(s.skill_id,data);if(state==='NOT_INTRODUCED'&&!Object.values(window.RainbowCurriculum?.aliases||{}).includes(s.skill_id))return;const alias=Object.entries(window.RainbowCurriculum.aliases).find(([,id])=>id===s.skill_id)?.[0];const ev=alias?(data.evidence?.[`${s.domain}::${alias}`]||[]):[];const recent=ev.slice(-10),avg=recent.length?recent.reduce((n,x)=>n+(+x.score||0),0)/recent.length:0;rows.push({learner_id:learnerId,domain:s.domain,subdomain:s.stage,skill_id:s.skill_id,skill_name:s.title,skill_level:s.difficulty_level,mastery_state:state,mastery_score:avg,confidence_score:Math.min(1,recent.length/5),attempt_count:recent.length,last_practiced_at:recent.length?`${recent[recent.length-1].date}T12:00:00Z`:null,difficulty_level:s.difficulty_level});});if(rows.length){const result=await client.from('learner_skill_progress').upsert(rows,{onConflict:'learner_id,skill_id'});if(result.error)throw result.error;}}
async function appendAttempt(evt,activity){if(!remote||!learnerId||!evt||!activity)return;try{await beginSession(window.data||{});const skillId=window.RainbowCurriculum?.aliases?.[activity.skill]||`${activity.domain}.${String(activity.skill).toLowerCase().replace(/[^a-z0-9]+/g,'_')}`;const result=evt.type==='success'?'CORRECT':evt.type==='not_yet'?'INCORRECT':'PARTIAL';const saved=await client.from('learning_attempts').upsert({attempt_id:evt.event_id||uuid(),learner_id:learnerId,session_id:sessionId,activity_id:activity.id,domain:activity.domain,skill_id:skillId,interaction_type:activity.interaction_type||activity.type||'offline_activity',difficulty:activity.difficulty||1,question_or_task_id:activity.id,result,hint_level_used:evt.hintLevel||0,number_of_attempts:evt.attempts||1,response_time_ms:evt.elapsed?evt.elapsed*1000:null,completed:evt.type==='success'||evt.type==='not_yet',timestamp:evt.timestamp||new Date().toISOString(),curriculum_version:window.RainbowCurriculum?.version||'2026.09.07-v1'},{onConflict:'attempt_id'});if(saved.error)throw saved.error;log('PROGRESS_SAVE_SUCCESS',{kind:'attempt',activityId:activity.id});}catch(error){log('PROGRESS_SAVE_FAILED',{kind:'attempt',activityId:activity.id,message:error.message});throw error;}}
async function syncSnapshot(data){if(!remote||!learnerId)return false;try{const profile=await client.from('learner_profiles').update({preferred_themes:[data.theme||'unicorn'],last_activity_at:new Date().toISOString(),curriculum_version:window.RainbowCurriculum?.version||'2026.09.07-v1'}).eq('learner_id',learnerId);if(profile.error)throw profile.error;await syncSkills(data);localStorage.removeItem(PENDING_KEY);log('PROGRESS_SAVE_SUCCESS',{kind:'snapshot'});return true;}catch(error){log('PROGRESS_SAVE_FAILED',{kind:'snapshot',message:error.message});throw error;}}
function queueSync(data){const snapshot=clone(data);localStorage.setItem(PENDING_KEY,JSON.stringify(snapshot));clearTimeout(syncTimer);syncTimer=setTimeout(()=>syncSnapshot(snapshot).catch(()=>{}),600);}
function backendStatus(){return {ready,remote,learnerId,sessionId,configured:!!(cfg().supabaseUrl&&cfg().supabaseAnonKey)};}
window.RainbowPersistence={init,hydrate,signIn,signOut,ensureProfile,beginSession,appendAttempt,queueSync,syncSnapshot,backendStatus,uuid,CACHE_KEY,PENDING_KEY};
})();
